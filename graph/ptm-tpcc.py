...
tpcc_type_array = ['TPCC_Light']
cc_type_array = ['Courier', 'Courier_SAVE', 'SP', 'Timestone', 'Zen-TICTOC', 'Zen-MVCC', 'Zen-SILO']
...
for idx, tpcc_type in enumerate(tpcc_type_array):
    fig = plt.figure()

    tpcc_table = get_workload_table(total_table, tpcc_type)

    for cc in cc_type_array:
        cc_table = get_cc_table(tpcc_table, cc)
    ...
    plt.savefig('fig/ptm-tpcc-' + tpcc_type + '.pdf', bbox_inches='tight')

lines, labels = fig.axes[-1].get_legend_handles_labels()
legend = fig.legend(lines, labels, bbox_to_anchor=(0.9, 1.1), fontsize=14, ncol=8, framealpha=0)
export_legend(legend, 'fig/ptm-tpcc-legend.pdf')
