...
ycsb_type_array = ['YCSB_A', 'YCSB_B', 'YCSB_C', 'ReadMostly', 'WriteIntensive', 'WriteMostly']
...
line_graph_properties = {'Courier': LineGraphProperty('Courier', 'o', '#63b2ee', '-'),
                         ...
                         'Timestone': LineGraphProperty('Timestone', 'v', '#7cd6cf', '-'),
...
for idx, ycsb_type in enumerate(ycsb_type_array):
    ...
    for cc in cc_type_array:
        ...
        throughputs = cc_table['Latency(99%) ns']
        plt.plot(
            thread_nums,
            throughputs,
    ...
    plt.xticks(thread_nums, fontsize=17)
    plt.yticks(fontsize=17)

    plt.savefig('fig/ptm-latency-' + ycsb_type + '.pdf', bbox_inches = 'tight')

# Legend
lines, labels = fig.axes[-1].get_legend_handles_labels()
legend = fig.legend(lines, labels, bbox_to_anchor=(0.9, 1.1), fontsize=14, ncol=8, framealpha=0)
export_legend(legend, 'fig/ptm-latency-legend.pdf')