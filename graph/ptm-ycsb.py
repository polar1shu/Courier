...
cc_type_array = ['Courier', 'Courier_SAVE', 'SP', 'Timestone', 'Trinity', 'Zen-TICTOC', 'Zen-MVCC',
                 'Zen-SILO']

#63b2ee - 76da91 - f8cb7f - f89588 - 7cd6cf - 9192ab - 7898e1 - efa666 - eddd86 - 9987ce - 63b2ee - 76da91
line_graph_properties = {'Courier': LineGraphProperty('Courier', 'o', '#ff5959', '-'),
                         'Courier_SAVE': LineGraphProperty('Courier-Save', 'o', '#facf5a', '-'),
                         'SP': LineGraphProperty('SP3', 's', '#49beb7', '-'),
                         'Trinity': LineGraphProperty('Trinity', '^', '#085f63', '-'),
                         'Timestone': LineGraphProperty('Timestone', 'v', '#10316b', '-'),
                         'Zen-TICTOC': LineGraphProperty('Zen-Tictoc', 'd', '#0b8457', '--'),
                         'Zen-MVCC': LineGraphProperty('Zen-MVCC', 'd', '#3d6cb9', '--'),
                         'Zen-SILO': LineGraphProperty('Zen-SILO', 'd', '#00d1ff', '--')}
...
for idx, ycsb_type in enumerate(ycsb_type_array):
    ...
    plt.savefig('fig/ptm-ycsb-' + ycsb_type + '.pdf', bbox_inches='tight')

# Legend
lines, labels = fig.axes[-1].get_legend_handles_labels()
legend = fig.legend(lines, labels, bbox_to_anchor=(0.9, 3), markerscale=2, labelspacing=5, fontsize=18, ncol=4,
                    framealpha=0)
export_legend(legend, 'fig/ptm-ycsb-legend.pdf', [-5, -50, 5, 50])
