...
namespace ix {
	...
	inline namespace bptree {
		...
		constexpr int QUERY_NUM = 25;

		constexpr auto IS_FORWARD = [](auto c){
			return (c & 0x1) == 0;
		};
		...
		public:
			...
			void btree_insert(entry_key_t, char *, const auto &);

			void btree_insert_internal(char *, entry_key_t, char *, uint32_t, const auto &);

			void btree_delete(entry_key_t);

			void btree_delete_internal(
					entry_key_t, char *, uint32_t, entry_key_t *,
					bool *, page **
			);
		...
		private:
			...
			uint8_t is_deleted;     // 1 byte
			int16_t last_index;     // 2 bytes
			std::mutex *mtx;        // 8 bytes


		public:
			header():   leftmost_ptr(nullptr), sibling_ptr(nullptr),
			            switch_counter(0), is_deleted(false),
			            last_index(-1), mtx(new std::mutex()) {}
		...
		public:
			entry(): key(std::numeric_limits<entry_key_t>::max()), ptr(nullptr) {}
		};

		constexpr int cardinality = (PAGESIZE - sizeof(header)) / sizeof(entry);
		constexpr int count_in_line = CACHE_LINE_SIZE / sizeof(entry);
		...
		public:
			...
			explicit page(uint32_t level = 0) {
				hdr.level      = level;
			...
			page(page *left, entry_key_t key, page *right, uint32_t level = 0) {
				...
				NVM::pwb_range((char *) this, sizeof(page));
			}
			...
			inline bool remove_key(entry_key_t key) {
				...
				for (i = 0; records[i].ptr != nullptr; ++i) {
					...
					if (shift) {
						...
						if (do_flush) {
							NVM::pwb_range((char *) records_ptr, CACHE_LINE_SIZE);
						}
					}
				}
			...
			bool remove(
					btree *bt, entry_key_t key, bool only_rebalance = false,
					bool with_lock = true
			) {
				hdr.mtx->lock();
			 ...
			 */
			bool remove_rebalancing(
					btree *bt, entry_key_t key, const auto &page_allocate,
				...
				if (!only_rebalance) {
					...
					if (this == (page *) bt->root) {
						if (hdr.level > 0) {
							if (num_entries_before == 1 && !hdr.sibling_ptr) {
								...
								NVM::pwb_range((char *) &(bt->root), sizeof(char *));
					...
					if (!should_rebalance) {
						...
						return hdr.leftmost_ptr != nullptr || ret;
					}
				}
				...
				page *left_sibling;
				bt->btree_delete_internal(
						key, (char *) this, hdr.level + 1,
				...
				if (is_leftmost_node) {
					...
					hdr.sibling_ptr->remove(
							bt, hdr.sibling_ptr->records[0].key, true,
							with_lock
					);
					if (!with_lock) {
					...
					if (num_entries < left_num_entries) { // left -> right
						if (hdr.leftmost_ptr == nullptr) {
							for (int i = left_num_entries - 1; i >= m; i--) {
								insert_key(
										left_sibling->records[i].key,
										left_sibling->records[i].ptr, &num_entries
								);
							}

							left_sibling->records[m].ptr = nullptr;
							NVM::pwb_range((char *) &(left_sibling->records[m].ptr), sizeof(char *));

							left_sibling->hdr.last_index = m - 1;
							NVM::pwb_range((char *) &(left_sibling->hdr.last_index), sizeof(int16_t));

							parent_key = records[0].key;
						}
						else {
							insert_key(
									deleted_key_from_parent, (char *) hdr.leftmost_ptr,
									&num_entries
							);
							...
							NVM::pwb_range((char *) &(hdr.leftmost_ptr), sizeof(page *));
						...
						if (left_sibling == ((page *) bt->root)) {
							...
							bt->btree_insert_internal((char *) left_sibling, parent_key,
							                          (char *) this, hdr.level + 1, page_allocate
							);
						}
					}
					else { // from leftmost case
						hdr.is_deleted = 1;
						NVM::pwb_range((char *) &(hdr.is_deleted), sizeof(uint8_t));
						...
						if (hdr.leftmost_ptr == nullptr) {
							for (int i = 0; i < num_dist_entries; i++) {
								left_sibling->insert_key(
										records[i].key, records[i].ptr,
										&left_num_entries
								);
							}
							...
							NVM::pwb_range((char *) (new_sibling), sizeof(page));

							left_sibling->hdr.sibling_ptr = new_sibling;
							NVM::pwb_range((char *) &(left_sibling->hdr.sibling_ptr), sizeof(page *));
						...
						if (left_sibling == ((page *) bt->root)) {
							...
							bt->btree_insert_internal((char *) left_sibling, parent_key,
							                          (char *) new_sibling, hdr.level + 1, page_allocate
							);
						}
			...
			inline void insert_key(
					entry_key_t key, char *ptr, int *num_entries,
				...
				if (*num_entries == 0) { // this page is empty
					...
					if (flush) {
						NVM::pwb_range((char *) this, CACHE_LINE_SIZE);
					}
				}
				else {
					...
					if (flush) {
						if ((uint64_t) &(records[*num_entries + 1].ptr) % CACHE_LINE_SIZE == 0) {
							NVM::pwb_range((char *) &(records[*num_entries + 1].ptr), sizeof(char *));
						}
					}
					...
					for (i = *num_entries - 1; i >= 0; i--) {
						if (key < records[i].key) {
							...
							if (flush) {
								NVM::pwb_range((char *) &records[i + 1], sizeof(entry));
							}
							inserted = 1;
							break;
						}
					...
					if (inserted == 0) {
						...
						records[0].ptr = ptr;
						if (flush) {
							NVM::pwb_range((char *) &records[0], sizeof(entry));
						}
					}
				}
			...
			page *store(
					btree *bt, char *left, entry_key_t key, char *right, bool flush,
					bool with_lock, const auto &page_allocate, page *invalid_sibling = nullptr
			) {
				if (with_lock) {
				...
				if (hdr.sibling_ptr && (hdr.sibling_ptr != invalid_sibling)) {
					...
					if (key > hdr.sibling_ptr->records[0].key) {
						...
						return hdr.sibling_ptr->store(
								bt, nullptr, key, right, true, with_lock, page_allocate,
								invalid_sibling
						);
					}
				}
				...
				if (num_entries < cardinality - 1) {
					...
					if (hdr.leftmost_ptr == nullptr) { // leaf node
						for (int i = m; i < num_entries; ++i) {
							sibling->insert_key(
									records[i].key, records[i].ptr, &sibling_cnt,
									false
							);
						}
					}
					...
					NVM::pwb_range((char *) sibling, sizeof(page));

					hdr.sibling_ptr = sibling;
					NVM::pwb_range((char *) &hdr, sizeof(hdr));
					...
					NVM::pwb_range((char *) &records[m], sizeof(entry));

					hdr.last_index = m - 1;
					NVM::pwb_range((char *) &(hdr.last_index), sizeof(int16_t));
					...
					if (bt->root == (char *) this) { // only one node can update the root ptr
						page *new_root = page_allocate();
						new (new_root) page((page *) this, split_key, sibling, hdr.level + 1);
						bt->setNewRoot((char *) new_root);
						...
						bt->btree_insert_internal(
								nullptr, split_key, (char *) sibling,
								hdr.level + 1, page_allocate
						);
					}
			...
			void linear_search_range(
					entry_key_t min, entry_key_t max,
					unsigned long *buf
			) {
				int i, off = 0;
				uint8_t previous_switch_counter;
		 ...
		 * class btree
		 */
		btree::btree(const auto &page_allocate) {
			root = (char *)page_allocate();
			new(root) page;
		...
		void btree::setNewRoot(char *new_root) {
			...
			NVM::pwb_range((char *) &(this->root), sizeof(char *));
			++height;
		}
		...
		void btree::btree_insert(entry_key_t key, char *right, const auto &page_allocate) { // need to be string
			page *p = (page *) root;
			...
			if (!p->store(this, nullptr, key, right, true, true, page_allocate)) { // store
				btree_insert(key, right, page_allocate);
			}
		}

// store the key into the node at the given level
		void btree::btree_insert_internal(
				char *left, entry_key_t key, char *right,
				uint32_t level, const auto &page_allocate
		) {
			...
			if (!p->store(this, nullptr, key, right, true, true, page_allocate)) {
				btree_insert_internal(left, key, right, level, page_allocate);
			}
		}
		...
		void btree::btree_delete_internal(
				entry_key_t key, char *ptr, uint32_t level,
